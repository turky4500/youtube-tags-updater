...
def get_trending_keywords():
    ...
    try:
        ...
        if not final_keywords: raise ValueError("ما لقينا ترند عربي")
...
def get_videos_to_update(youtube, update_log):
    ...
    for item in response['items']:
        ...
        if last_update_str:
            ...
            if last_update > cutoff_date: continue
...
def smart_tag_selection(video_title, all_trends):
    ...
    for trend in all_trends:
        ...
        if video_words & trend_words: related_tags.append(trend)
...
def update_video(youtube, video_id, video_title, new_tags):
    try:
        ...
        res = req.execute()
        if not res['items']: return False, "الفيديو غير موجود"
